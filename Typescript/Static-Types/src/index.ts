...
let myName: string = "Amot the Dev";
...
function writeTask(taskName: string): void{
...
function stripText(word: any): any{
...
function calculateTax_f(amount: number, format: boolean): number | string{
    return format ? `Tax: ${amount * 1.2}` : amount
}

let taxValue : string | number = calculateTax_f(200, true)  // this variable belongs to type number and string
...
let newTaxValue = calculateTax_f(299, true) as string;
...
function findHighestNumber(format: boolean = false, ...nums: number[]): string | number{
    let highestNumber: number = Math.max(...nums)
...
function calculateCartItems(items: number[], format: boolean = false): string | number{
    if(items.length == 0){
        // return null  // here there wont be error because null types is a valid type of any data type
        return 0
    }else{
        let sum = items.reduce((prev: number, newItem: number) => prev + newItem)
        return format ? `Items in Cart is ${sum}` : sum; 
    }
}

taxValue = calculateCartItems([]) as number
console.log(taxValue - 10)

// USING NULL REMOVAL FROM UNION TYPES 
// using union types provides the intersection API between each of the types that is being joined together 
// null and undefined has not property and this means that if they are being used in intersection with other types, there wont be any common 
// API thus making the variable not have any property. This can be removed by union removal syntax
type User  = {
    name: string,
    username: string,
...
function getUserContactDetails(user: User): string | number | null{
